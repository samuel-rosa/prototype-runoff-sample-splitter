...
if (!file.exists("../data/vector/brasil.shp")) {
  ...
  brasil <- sf::st_read("../data/vector/brasil.shp")
} else {
  brasil <- sf::st_read("../data/vector/brasil.shp")
}
...
# Load soil map
rs <- 
  sf::st_read("../data/vector/solo-rs.shp") %>% 
...
where _i_ = 1, 2, ..., _n_, with _n_ = 5, and _j_ is one of the three artificial suspensions. The percentual mean estimation error (ME), defined as the percentage magnitude of the estimation error in relation to the true concentration in a suspension _j_, was calculated with

$ME_j = \sum_{i = 1}^{n = 5} e_{ij}/y_j \times 100$

Finally, the estimation errors were submitted to a one sample, two-tailed _t_-test to test if its mean was equal to zero (H~0~: $\mu$ = 0) or, alternatively, less or greater than zero (H~1~: $\neq$ 0). The _t_-statistic was given by 

$t_j = \frac{\bar{e}_j}{s_j/\sqrt{5}}$
...
$t_j = \frac{\bar{d}_{j}}{s_{dj}/\sqrt{5}}$

where $\bar{d}$ is the mean of differences between estimation errors of outlets A and B, _s~dj~_ is the sample standard deviation of the differences, and _j_ is one of the four volumes tested.
...
Em comparação, @ZoebischEtAl1996 avaliaram cinco profissionais experientes em amostragem de enxurrada e
observaram erros na estimativa de perda de solo entre 5 e 83%.
