...
e <- 
  ...
  group_by(entrada) %>% 
  summarise('Média (g L^-1^)' = mean(solidos_totais), 'DP (g L^-1^)' = sd(solidos_totais), 'CV (%)' = sd(solidos_totais) / mean(solidos_totais) * 100, 
            'EMR (%)' = mean(erro), 'EP~EMR~ (%)' = sd(erro) / sqrt(5))
...
pander(d, caption = 'Dados e estatísticas da concentração de sólidos totais nas amostras obtidas usando o método do mergulho de três suspensões com concentração conhecida de sólidos totais de 2, 10 e 50 g L^-1^.')
```
