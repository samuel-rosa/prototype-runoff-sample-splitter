...
# Preparar tabela com dados físicos e químicos do perfil
perfil <- febr::layers("ctb0013", which.cols = "all", progress = FALSE)[[1]]
perfil_fis <- dplyr::select(perfil, -c(1:3, 5, 8, 16:ncol(perfil)))
perfil_qui <- dplyr::select(perfil, c(4, 6, 7, 16:ncol(perfil)))
colnames(perfil_qui) <- colnames(perfil_fis)
rbind(perfil_fis, colnames(perfil)[c(4, 6, 7, 16:ncol(perfil))], perfil_qui) %>% 
  knitr::kable(
    booktabs = TRUE,
