...
# Descarregar dados do febr e preparar unidades
...
col_nam <- glue::glue("{col_nam} {attr(perfil, 'units')[idx]}")
colnames(perfil_fis) <- col_nam
...
```{r solos-do-rs, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Área do estado do Rio Grande do Sul com camada superficial do solo de textura similar ao material do solo usado neste estudo.", fig.align='center'}
rs <- raster::shapefile("../data/solo_res_2005.shp", stringsAsFactors = TRUE) %>% 
  sp::spTransform(sp::CRS("+proj=longlat +ellps=WGS84"))
rs$col <- "gray95"

# Avaliação dos polígonos
# A função 'grepl' retorna '1' quando atende ao critério, e '0' caso contrário.
# Assim, somamos os resultados para agregar os dados. Quanto maior o valor final,
# maior é o número de critérios ao qual um polígono atende.
idx <- 
  grepl("Argissolo", rs$SOLO_EMBRA) +
  grepl("Planossolo", rs$SOLO_EMBRA) +
  grepl("aren", rs$MAT_ORIGEM) +
  grepl("arenosa", rs$TEXTURA) +
  grepl("com", rs$GRADIE_TEX) +
  # Critérios a descontar
  grepl("abrupto", rs$GRADIE_TEX) -
  grepl("basalt", rs$MAT_ORIGEM) -
  grepl("argil", rs$MAT_ORIGEM)

# Número mínimo de critérios aos quais um polígono precisa atender
rs$col[idx > 3] <- "gray25"
sp::spplot(
  rs, "SOLO_EMBRA", grid = TRUE, col.regions = colorRampPalette(rs$col)(length(rs$col)), col = NA, 
  colorkey = FALSE, scales = list(draw = TRUE), ylab = "Latitude", xlab = "Longitude",
  panel = function (...) {
    sp::panel.polygonsplot(...)
    lattice::panel.grid(v = -1, h = -1)
    lattice::panel.points(-53.790215, -29.657668, pch = 4, col = "black")
    lattice::panel.text(-53.790215, -29.657668, "Santa Maria", pos = 3)
  })
```
